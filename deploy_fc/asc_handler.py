    ...
    if consolidation:
        for schema in consolidation:
            if hasattr(schema, "model_dump"):
                try:
                    consolidation_dicts.append(schema.model_dump(mode="json"))
                except TypeError:
                    # Pydantic v1 fallback
                    d = schema.dict() if hasattr(schema, "dict") else schema.model_dump()
                    consolidation_dicts.append(json.loads(json.dumps(d, default=str)))
            elif hasattr(schema, "to_dict"):
                consolidation_dicts.append(json.loads(json.dumps(schema.to_dict(), default=str)))
            else:
...
def handler(event, context):
    ...
    try:
        ...
        logger.info("psycopg2 import check: SUCCESS")
    except Exception as err:
        logger.exception("psycopg2 import check: FAILED")
    ...
    try:
        ...
        logger.info("ASC handler complete: %s", json.dumps(summary, default=str))
