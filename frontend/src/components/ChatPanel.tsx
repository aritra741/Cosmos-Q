    ...
    <div className="flex flex-col h-full bg-zinc-950 border-r border-zinc-800">
                ...
                >
                  <div
                    className={cn(
                      ...
                      msg.role === 'user' ? 'text-zinc-500' : 'text-emerald-500/70'
                    )}
                  >
                    {msg.role}
                  </div>
      ...
      <div className="p-4 bg-zinc-950 border-t border-zinc-800 space-y-3">
        ...
        {mode === 'demo' && isComplete ? (
          ...
          <>
            {mode === 'demo' ? (
              <div className="w-full flex items-center gap-2.5 bg-zinc-900/60 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-400">
                <SparklesIcon size={14} className="text-emerald-500 shrink-0" />
                <span className="truncate">
                  {isBusy
                    ? `Processing${nextChip ? ` · ${nextChip}` : '…'}`
                    : nextChip
                    ? `Ready · ${nextChip}`
                    : 'Session ready'}
                </span>
